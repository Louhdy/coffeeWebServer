...
exports.connectToNetwork = async function (userName) {    
  try {
    ...
    const contract = await network.getContract('coffeContracts');    
...
exports.getAllReceptions = async function (networkObj, func) {
  try {
    let response = await networkObj.contract.submitTransaction(func, id);
    console.log(response);
    console.log(`Transaction ${func} with args has been submitted`);
    await networkObj.gateway.disconnect();
    return response;
...
exports.invoke = async function (networkObj, isQuery, func, id, args) {
  try {
